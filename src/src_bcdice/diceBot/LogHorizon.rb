...
　x:CRを指定。
　±y:修正値。＋と－の計算に対応。省略可能。
　$z：＄を付けると骰子目を z 固定。表の特定の値参照用に。省略可能。
　例） PCT1　ECT2+1　GCT3-1　CCT3$5
・財宝表(tTRSx±y$)
　CTRS 金銭／MTRS 魔法素材／ITRS 換金アイテム／OTRS そのほか／※HTRS ヒロイン／GTRS ゴブリン財宝表
　x：CRを指定。省略時は骰子値 0 固定で修正値の表参照。《ゴールドフィンガー》使用時など。
　±y：修正値。＋と－の計算に対応。省略可能。
　$：＄を付けると財宝表の骰子目を7固定（1回分のプライズ用）。省略可能。
　例） CTRS1　MTRS2+1　ITRS3-1　ITRS+27　CTRS3$
・パーソナリティタグ表(PTAG)
・交友表(KOYU)
・イースタル探索表(ESTLx±y$z)
　x：CRを指定。省略時は骰子値 0 固定で修正値の表参照。
　±y：修正値。＋と－の計算に対応。省略可能。
　$z：＄を付けると骰子目を z 固定。特定CRの表参照用に。省略可能。
...
MESSAGETEXT
  ...
  def getCurseSpecialConsumptionResultTables
    ...
    tables = [[
      ...
      [3, '種族変更の呪い：ランダムで骰子を振って別の種族に変わる。種族特技が使用不能になる。'],
  ...
  def getPrefixedMagickItemDiceCommandResult(command)
    ...
    table_1 = [
      ...
      "接頭語：必殺の　対応タグ：すべての武器\nアイテム効果：〔起動：判定直後〕この武器による[武器攻撃]による[命中判定]の骰子に６の出目が１つ以上あれば、判定をクリティカルにする。シナリオ１回使用可能。",
