...
----------------------------------------------------------------
以下のコマンドは、リヴラデッキ卡牌の補助を目的としたものです。
使用の際は上部メニューより
「卡牌　→　卡牌配置の初期化　→　紫縞のリヴラドール：リヴラデッキ」
と操作し、リヴラデッキを使用できる状態にしておいてください。

各リヴラデッキ卡牌の末尾に書かれている [] 内のコマンドをタイプすると、
卡牌名と効果のテキストを参照できます。
    ...
    when 'C'
      ...
      when 'L'
        table = [
          ...
          [4, '『ウォーク』　【ムーヴ：１】を行う。'],
          [5, '『ラン』　【ムーヴ：２】を行う。'],
          [6, '『オルタネート』　直後に行うアタックネイルでは、骰子ロールを２度行い、任意の一方の結果を選ぶことが出来る。'],
          [7, '『ジャンプ』　このアタックネイル終了時まで自身が「高度：３《ＯＤ》」に存在するものとして扱う。'],
          [8, '『オーヴァードライヴ』　直後のネイルでは《ＯＤ》指定された数値を「４《ＯＤ》」増加する。'],
          [9, '『ガード』　骰子ロールで使用された骰子の内ふたつまでを選ぶ。その出目を１減少する。'],
          [10, '『パリィ』　骰子ロールで使用された骰子の内ふたつまでを選ぶ。その骰子を振り直しさせる。この時、振り直した後の値のみを採用する。'],
          ...
          [14, '『ライトニングダンス』　即座に【ムーヴ：１】を行う。（これによって、直後のアタックネイルの射程の外に出た場合、そのアタックネイルの骰子ロールは行われず、失敗したことになる）'],
        ]
      when 'D'
        table = [
    ...
    when 'K'
      ...
      when 'L'
        table = [
          ...
          [9, '『舞い踊る黒刃（ハイマニューバ・ブラック・ブレイド）』　直前のムーヴネイルで移動したセル数に等しい回数だけ、直後のアタックネイルの複製新增し、実行する。'],
      ...
      when 'D'
        table = [
          ...
          [3, '『第三夜の黒（ハーフブラック・ドレス）』　自身がドロゥを行う時、骰子ロールを行う骰子のうちひとつを振らず、任意の出目を出したものとして良い。'],
      ...
      when 'O'
        table = [
          [1, '『意志の介入（マインド・ランペイジ）』　玩家の１人を対象とする。対象のオーナーズネイルをすべて見る。その後、その中のひとつを「使用済」にする。'],
          [2, '『意志の散逸（マインド・ロスト）』　ドロゥの骰子ロールに使用された骰子ひとつを取り除く。（取り除かれた骰子はセット骰子としてセットされない）'],
          ...
          [7, '『傷跡の共鳴（ハート・レゾナンス）』　玩家１人を対象とする。対象の「シールドレス」を１枚破壊する。その後自身の「シールドレス」を１枚破壊する。'],
          [8, '『居心地の悪さ（ブラック・マイアズマ）』　このムーヴネイルを打ち消す。'],
          [9, '『鉄茨よ侵食せよ（ブラックブランブル）』　セルひとつを指定する。指定したセルに「鉄茨マーカー」を設置する。'],
          [10, '『鉄華乱舞（アイアン・ブルーム）』　ユニット１体を対象とする。対象に【アタック：３】を行う。これによって対象の「シールドレス」を１枚以上破壊した場合、対象のセット骰子の内、貴女の任意のふたつを取り除く。'],
          ...
          [15, '『雷電の黒絢（ライトニング：ザ　ブラックソード）』　ユニット１体を対象とする。対象に【アタック：２】を行う。これによって対象の「シールドレス」を１枚以上破壊した場合、さらに対象のセット骰子を１個取り除く。'],
          [16, '『思索の黒絢（マインド：ザ　ブラックソード）』　全ての玩家は玩家自身のセット骰子を１個取り除く。その後貴女は【ドロゥ：２】を行う。'],
          [17, '『爛熱の黒絢（グロウス：ザ　ブラックソード）』　全ての玩家は【ドロゥ：１】を行う。その後、貴女は【ドロゥ：２】を行う。'],
        ]
      end
    when 'W'
      case cardtype
      when 'L'
        table = [
          ...
          [9, '『輝きの盾（ホワイトシールド）』　自身のパッシヴドレスの「ブロックナンバー」に６を追加する。この効果はこの骰子ロールの結果のみに有効である。'],
          [10, '『より疾きは光の一手（ライトニング・インターセプト）』　移動を行ったユニットに対し、即座に【アタック：２】を行う。'],
          [11, '『白の語り部（ホワイトテラー）』　骰子ロールで使用された骰子全ての出目を６に変更する。'],
        ]
      when 'D'
        table = [
          [1, '『愚者の白（ホワイトフール）』'],
          [2, '『魔術師の白（ホワイトマジシャン）』　自身が「高度：６」以上に存在する時、ドロゥ数を＋１しても良い。'],
          [3, '『女教皇の白（ホワイトハイプリエステス）』　１ラウンドに１回まで、自身の骰子ロールの出目ひとつを１増加しても良い。'],
          [4, '『女帝の白（ホワイトエンプレス）』　自身の白のアタックネイルに、以下の一文を追加する。「続けて【アタック：１】を行う。射程はこのアタックネイルに準ずる」'],
          [5, '『皇帝の白（ホワイトエンペラー）』　自身の「シールドレス」が破壊される度に、１ｄ６の骰子ロールを行う。この時１か２の出目が出た場合「シールドレス」を１枚回復する。'],
        ]
      when 'O'
        table = [
          [1, '『秩序の護り手（ホワイト・ディフェンダー）』　【ドロゥ：１】を行う。'],
          [2, '『あなたにも愛を（トゥーミーユアラヴリィ）』　自身の「シールドレス」を１枚回復し、その後自身以外の「シールドレス」を１枚回復する。'],
          [3, '『罪なき純白（じゅんぱくイノセント）』　このドロゥフェイズで行う骰子ロールの出目を全て１減少する。（１の出目は１のままである）'],
          ...
          [10, '『忘却の白（ホワイト・オブリビオン）』　自身のセット骰子を全て取り除く。自身のオーナーズネイルの内、任意のふたつの「使用済」を解除する。'],
          [11, '『白の従者（ホワイト・フォロワ）』　セルひとつを指定する。指定したセルに「白従者マーカー」を設置する。'],
          [12, '『秩序の龍（クロム・クラーク・レプリカ）』　セルひとつを指定する。指定したセルに「偽龍マーカー」を設置する。'],
          [13, '『夢の向こうの旅人（ロアテラ）』　このドロゥフェイズで行う骰子ロールでは、骰子それぞれに対し、任意の出目が出たものとして扱う。'],
          ...
          [17, '『混色もまた物語：緑（ロア：グリーン）』　玩家１人を対象とする。対象の「シールドレス」を１枚回復する。'],
        ]
      end
    when 'R'
      case cardtype
      when 'L'
        table = [
          ...
          [7, '『追い打ちの炎渦（ファイアストーム）』　直後に行うアタックネイルでは、《ＯＤ》指定された数値を「３」増加する。また、このアタックネイルで「シールドレス」を１枚以上破壊した場合、即座に【ドロゥ：２】を行い、続けてセット骰子を２個取り除く。'],
      ...
      when 'O'
        table = [
          [1, '『走れ雷電（ライトニング・ボルト）』　ユニット１体を対象とし、【アタック：２】を行う。全ての玩家は、このオーナイズネイルに対し、リアクトネイルを使用することが出来ない。'],
          [2, '『穿て炎槍（フレイムランス）』　ユニット１体を対象とし、【アタック：２】を行う。全ての玩家は、このオーナイズネイルに対し、リアクトネイルを使用することが出来ない。'],
          [3, '『熱情と踊れ（ダンス・ウィズ・ヒート）』　自身のリヴラドールを含む全てのユニットに対し【アタック：１】を行う。'],
          [4, '『昇炎の罠（ファイアリングトラップ）』　直後のムーヴネイルで移動を行ったユニットを対象とし、【アタック：３】を行う。'],
          [5, '『精神混沌の炎（レッド・パラノイア）』　自身のセット骰子のうち、任意のふたつを取り除き、【ドロゥ：２】を行う。セット骰子がふたつ以上存在しない場合にはこのネイルを使用することが出来ない。'],
          ...
          [8, '『咲き乱れよ百合の花（レッド・リリィ）』　自身を含む全てのユニットが行ったドロゥの骰子ロール結果全てを振り直させる。'],
          [9, '『煉獄の恋（ヘルフレイム・ラヴソング）』　玩家１人を対象とする。対象の「シールドレス」を１枚破壊する。その後自身の「シールドレス」を１枚破壊する。'],
          ...
          [13, '『再臨の銀（アガートラム）』　このドロゥフェイズで行った自身の骰子ロール結果の骰子全ての出目を３減少する。その後【ドロゥ：３】を行う（このドロゥには出目減少の効果は適用されない）。'],
          [14, '『銀腕、携えるは黒（フレイガラク：ブラック）』　自身のセット骰子を２個取り除く。ユニット１体を対象とする。対象に【アタック：３】を行い、【アタック：２】を行い、【アタック：１】を行う。'],
          [15, '『銀腕、携えるは白（クライドハームソルース：ホワイト）』　全てのユニットの「シールドレス」を１枚回復する。その後貴女はさらに「シールドレス」を１枚回復する。'],
          [16, '『銀腕、携えるは青（カレトヴルッフ：ブルー）』　【ドロゥ：５】を行う。その後セット骰子を２個取り除く。'],
    ...
    when 'B'
      ...
      when 'L'
        table = [
          ...
          [9, '『思考の渦（ぐるぐる）』　骰子ロールに使用された骰子ひとつを指定する。その骰子を振り直させる。'],
          [10, '『空翔けの回避（レビテート）』　そのアタックネイルの骰子ロールで使用された骰子全ての出目を１減少する。'],
          [11, '『たゆたう心、空の様に（ストラトスフィア・ハート）』　自身に適用された【エクストルード】を打ち消し、元のセルへと戻る。その後【ドロゥ：１】を行う。'],
        ]
      when 'D'
        table = [
          [1, '『青空を這い（スカイ・クロウラ）』　セットアップフェイズ毎に、骰子を１個振っても良い。そうした場合、クリンナップフェイズまで、骰子の出目に等しい「高度」に自身が存在するものとして扱う。'],
      ...
      when 'O'
        table = [
          ...
          [2, '『入念な思考（これとこれは、いらないかな）』　【ドロゥ：１】を行い、セット骰子から任意のひとつを取り除く。'],
          [3, '『即決即断（みてたよ。させないんだから）』　この骰子ロールの骰子目全てを２減少する。'],
          [4, '『方針変更（こっちの方がきっといいよ）』　骰子ロールで使用された骰子ひとつを裏返す（もしくは７からその出目の数値を引いた出目に変更する）。'],
          [5, '『小さな知略（マハトマ）』　任意の玩家１人を対象とする。対象のオーナーズネイルを見る。その中から１枚を指定する。対象はそのオーナーズネイルを次のラウンドのクリンナップフェイズまで使用できなくなる。'],
          [6, '『青の精鋭（ブルー・アデプト）』　セット骰子を２個取り除く。ユニット１体を対象とする。対象に【アタック：２】を行う。'],
          [7, '『対抗（カウンタースペル）』　使用されたオーナーズネイルの効果は発揮されず、「使用済」となる。'],
          [8, '『碧空の加護（オルガ）』　自身のオーナーズネイルひとつの「使用済」を解除する。'],
          [9, '『思考妨害（あ、あれ見て？）』　骰子ロールに使用された骰子の内、最大ふたつまでを指定する。それらの骰子を振り直させる。'],
          [10, '『碩学式回路（ジーニアス・サーキット）』　【ドロゥ：３】を行い、セット骰子から任意のふたつを取り除く。'],
          [11, '『碩学式"大"回路（ジーニアス・メガ・サーキット）』　任意の玩家１人を対象とする。対象のオーナーズネイルを見る。その中から１枚を指定する。そのオーナーズネイルに以下の一文を追加する。「この効果を解決した後、自身に【アタック：５】を行う。」'],
          [12, '『偉大なる集合知（ハイアラキ）』　【ドロゥ：６】を行い、セット骰子から任意のみっつを取り除く。'],
          [13, '『碧空を越える者（ストラトスフィア・ブレイヴ）』　【ドロゥ：自身のリヴラドールの現在の高度】を行う。'],
          [14, '『深淵なる熟慮（わるだくみ）』　【ドロゥ：２】を行う。この時、６の出目を出した骰子はセットされず、取り除かれる。'],
          [15, '『深遠たる秩序（知識こそが正義）』　１～６の内、数字をひとつ指定する。このドロゥフェイズの間、全ての玩家が行う【ドロゥ：Ｘ】では、指定した出目を出した骰子はセットすることが出来ない。'],
    ...
    when 'G'
      ...
      when 'D'
        table = [
          ...
          [3, '『昇華の階段（スパイラル・アセンション）』　スタンバイフェイズ毎に、骰子を２個振り、セルをひとつ指定して良い。そうした場合、そのセルの高度は骰子ので目の合計値に変更される。'],
      ...
      when 'O'
        table = [
          ...
          [3, '『自然の叡智（ネイチャーズ・ウィズダム）』　アタックネイル、リアクトネイルのいずれか一方を指定する。全ての玩家はこのラウンド終了時まで、選択されたネイルを使用することが出来ない。'],
          ...
          [7, '『生命の芽吹き（カム・イントゥ・バッズ）』　セット骰子を２個取り除く。自身のシールドレスを１枚回復する。'],
          [8, '『絡めとり（まちなさい！）』　ユニット１体を対象とする。対象が「高度：１」以上の高度に存在する場合、対象の行う全ての骰子ロールの出目を１減少する。'],
          ...
          [12, '『吠え猛る龍禍（ワン・ゼイ・フィア）』　全てのユニットは、そのユニット自身に対して【アタック：そのユニットが存在する高度】を行う。この攻撃によって１枚以上シールドレスが破壊されたユニットは、自身のセット骰子を２個取り除く。'],
          [13, '『緩やかなる原初の監獄（エンクロージア）』　貴女を含む全ての玩家は、その玩家自身のシールドレスを１枚回復することを選んでも良い。その後、貴女はこれによって回復したシールドレスの合計枚数に等しい数のシールドレスをさらに回復する。'],
          ...
          [16, '『憤怒の大渦（アンガー・メイルストロム）』　全ての玩家のシールドレスを、現在最もシールドレスの枚数が少ない玩家の枚数と同じ枚数に変更する。'],
