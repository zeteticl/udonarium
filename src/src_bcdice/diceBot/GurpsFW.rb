...
・判定においてクリティカル・ファンブルの自動判別、成功度の自動計算。(3d6<=目標値)
 ・祝福等の骰子目にかかる修正は「3d6-1<=目標値」といった記述で計算されます。
 (骰子目の修正値はクリティカル・ファンブルに影響を与えません。)
...
・トラップリスト(TRAPd)
・報酬財宝桌面(xに到達深度を記述)。 (TRSdx)
 ・財宝桌面の段階が変動する場合、末尾に±yを記述(yは変動段階)。(TRSdx±y)
  [例：TRSE5-1、TRSH36+2]
・地形決定表(AREA)
・迷宮追加オプション表(RANDOP)
INFO_MESSAGE_TEXT
...
☆薄暗い(ストームコーザー「鳥目」を適用)',
    ...
    when "RANDOP"
      ...
      table = [
        '「宝物の迷宮」
財宝決定の際、1段階深度の高い財宝桌面を使用する。',
        '「密林の迷宮」　初期深度+10
分類が「魔獣」「獣人」「霊獣」の怪獸は全ての判定に+2のボーナスを得る。',
        '「カラクリ技師の迷宮」
...
リザードやワイズマンなどの寒さに弱い角色は動きが鈍り、あらゆる判定に-2のペナルティを受ける。',
        '「盗賊王の迷宮」　初期深度+10
<罠><鍵開け>に+3のペナルティを受ける。',
        '「ミミック狂暴化」
...
「分類：ギア」の怪獸は全ての判定とあらゆる攻撃の致傷力に+3のボーナスを得る。',
  ...
  # GURPS-FW 夢幻の迷宮財宝桌面(ver.2013/05/03)
  ...
  def getTrapTable(diff)
    ...
    table = [
      ...
      "ペトロブラスター：宝箱を開けた角色に《肉を石》をかける。技能レベルは#{['20', '22', '24', '30'][d]}。",
      ...
      "テレポーター：宝箱の周囲に存在するPC全員(とエンカウントしている怪獸)をダンジョン入口方面に転送する。深度が#{['3D減少する', '4D減少する', '5D減少する', '0になる'][d]}。",
      "アイスコフィン：宝箱を開けようとした角色に冷気で#{['3D', '5D', '7D', '12D'][d]}の「叩き」ダメージを与え更に最終的なダメージの半分のFPダメージを与える。",
      "クロスボウ：宝箱を開けようとした角色に#{['2D', '3D', '5D', '8D'][d]}の「刺し」ダメージを与える。盾の受動防御を無視した「よけ#{['', '-2', '-4', '-8'][d]}」で回避が可能。",
      "毒針：宝箱を開けようとした角色に#{['1D', '2D', '3D', '6D'][d]}の「刺し」ダメージを与える。1点でもダメージを受けると「生命力#{['-4', '-5', '-6', '-8'][d]}」で判定を行い、失敗すると1日間すべての判定に-2のペナルティを受ける。盾の受動防御を無視した「よけ#{['', '-2', '-4', '-8'][d]}」で回避が可能。",
      "アラーム：即座にその地形のエンカウント表(イベント表4-1～4-6)を振る。",
      "殺人鬼の斧：宝箱を開けようとした角色に#{['3D', '4D', '6D', '10D'][d]}の「切り」ダメージを与える。命中部位は「ランダム部位命中表」を用いて決定すること。盾の受動防御を無視した「よけ#{['', '-2', '-4', '-8'][d]}」で回避が可能。",
      "死神：宝箱を開けようとした角色にネクロマンサーの呪術【憑物】+【死神】の効果を与える。3ラウンドすべての判定に#{['-1', '-2', '-3', '-4'][d]}のペナルティを受け、効果が切れると同時に#{['3D+3', '3D+6', '3D+9', '3D+15'][d]}の防護点無視ダメージを受ける。",
...
----------",
      ...
      when 2
        table = [
          "財宝(カギ・トラップなし)。財宝の中身は1段階低い深度の財宝桌面を使用する。",
          "財宝(カギつき。行動を消費して#{getRandomDiff(dice1, 22, d, 0)}に成功すれば開く)財宝の中身は1段階低い深度の財宝桌面を使用する。",
          "財宝(カギなし、トラップつき。行動を消費して#{getRandomDiff(dice1, 23, d, 0)}に成功すれば解除可能)財宝の中身は1段階低い深度の財宝桌面を使用する。",
          "財宝(カギ、トラップつき。行動を消費して#{getRandomDiff(dice1, 24, d, 0)}に成功すれば解除可能)財宝の中身は1段階低い深度の財宝桌面を使用する。",
          "ミミックの罠。ランダムな角色に3Dの「切り」ダメージを与える。#{getRandomDiff(dice1, 25, d, 0)}に成功すればこの罠を見抜いて無効化することができ、なおかつ「深度×#{getRandomDiff(dice1, 25, d, 1)}」GPを入手可能。",
      ...
      when 3
        table = [
          ...
          "洞窟内で近道を発見。深度が1D増加する。細い道のため、体の大きな角色(STが13以上)は、通るのに苦労してFPを1点消耗する",
      ...
      when 4
        table = [
          ...
          "炎の燃え盛る洞窟。深度判定直前に炎で#{getRandomDiff(dice1, 45, d, 0)}の「叩き」ダメージを受ける。さらに#{getRandomDiff(dice1, 45, d, 1)}とエンカウント。火炎弱点の怪獸がいれば即座に逃亡する。",
          "冷気に包まれた洞窟。深度判定直前に冷気で#{getRandomDiff(dice1, 46, d, 0)}のFPダメージを受ける(防護点無視)さらに#{getRandomDiff(dice1, 46, d, 1)}とエンカウント。冷気弱点の怪獸がいれば即座に逃亡する。",
        ]

      when 5
        table = [
          "落盤が怪獸を押しつぶす。戦闘中の場合ランダムな怪獸1人を即死させる。APやドロップ品も入手可能。それ以外の場合は特に何も起きない。",
          ...
          "有毒ガスが発生している。分類「ギア」「アンデッド」以外の角色は敵味方全員#{getRandomDiff(dice1, 53, d, 0)}で判定を行い、失敗度分のダメージを受ける。《空気浄化》などがあれば使用することで無効化できる。",
          "溶岩の流れる部屋。深度判定直前に炎で#{getRandomDiff(dice1, 54, d, 0)}の「叩き」ダメージを受ける。深度判定を行い、失敗した角色はさらに追加で#{getRandomDiff(dice1, 54, d, 0)}の「叩き」ダメージを受ける。",
      ...
      when 6
        table = [
          ...
          "地熱で温泉が湧いている。このイベントが発生すると怪獸は消滅する。みんなで温泉に浸かることでFPが全回復する。また、温泉の薬効により受けていた毒は無効化され、ラウンドの最初にHPが#{getRandomDiff(dice1, 62, d, 0)}点ずつ回復するようになる。",
          "地下渓流を発見する。深度判定が体力判定ではなく<水泳>判定になる。深度判定に#{getRandomDiff(dice1, 63, d, 0)}成功以上した場合は地底湖に沈む財宝を発見可能。財宝にカギやトラップはなく、中身は深度そのままの財宝桌面を使用する。",
          ...
          "怪しい光を放つ鉱石の部屋。鉱石は無生物を破壊する振動を放っており、武器に#{getRandomDiff(dice1, 66, d, 0)}の効果を与える。また、種族「ギア」の角色も同様の効果を受ける。ラウンド終了前に鉱石を破壊すればこの効果は受けない。鉱石はどのヘクスからでも攻撃可能。分類：岩石、防護点8、HP#{getRandomDiff(dice1, 66, d, 1)}、切り刺しダメージボーナス無効。",
        ]
    ...
    when 2
      ...
      when 1
        table = [
          ...
          "「パワーアップ装置！」と書かれた怪しげな機械が置いてある。#{getRandomDiff(dice1, 16, d, 0)}を支払って使用すると角色を強化(？)可能。使用するかは自由だが、「好奇心」を持つ角色は知力判定に失敗すると使用する。使用した角色は1Dを振り、以下のような効果が発生する。
----------
...
----------",
      ...
      when 4
        table = [
          "#{getRandomDiff(dice1, 41, d, 0)}とエンカウント。2ラウンド経過するとキャンディークラウンは戦場全域に《べたべた》をかけて逃亡する。",
          "石像が並ぶ通路がある。1Dを振って1・2・3なら#{getRandomDiff(dice1, 42, d, 0)}。1Dを振って4・5・6・の場合ガーゴイルに似ている、ただの石像。<怪獸知識-5>か「第六感」の判定に成功していなければ必ず相手の先制攻撃になり、その際PC先手で進行中の場合も怪獸先手に変更される。#{getRandomDiff(dice1, 42, d, 1)}",
          "障害物に隠れて#{getRandomDiff(dice1, 43, d, 0)}が狙撃してくる。怪獸のすぐ傍には合計5ヘクスの障害物(防護点5、HP35。形はGMが任意に決定する)がある。障害物ごしに攻撃するには射撃武器でなくてはならず、射撃武器も命中に-2のペナルティを受ける。",
          "ブツゼンに怪獸が仁王立ちをしており、問答を行ってくる。#{getRandomDiff(dice1, 44, d, 0)}に成功すると戦闘を回避することができ、APとドロップ品も入手可能。失敗するか戦闘を望んだ場合#{getRandomDiff(dice1, 44, d, 1)}とエンカウントする。",
          "進路上に鎖に繋がれた怪獸を発見する。迂回して戦闘を避けるなら深度1D減少し、戦闘して倒すと深度が1D増加する。戦闘を行う場合#{getRandomDiff(dice1, 45, d, 0)}とエンカウントする。",
      ...
      when 5
        table = [
          ...
          "フロアイミテーターだ！この部屋自体が魔物である。ロックバトラー(#{getRandomDiff(dice1, 53, d, 0)})とエンカウントする。この怪獸がいる限り移動はできず、ラウンドごとの地形変化や深度増加も行わない。PCとフロアイミテーターは常に隣接しているものとして扱う。",
          ...
          "ラダマンティスの蛇の像と目が合う。どうやらダンジョンの監視者に見つかってしまったらしい！以後怪獸とエンカウントする場合、その数が2倍となる(この効果は累積する)なお、ネームドもしくは魔将の数は変化しない。",
          "怪しげな卵が1つ置かれている。GMは#{getRandomDiff(dice1, 56, d, 0)}の中から好きなものを1体選びエンカウントさせることができる。深度が0の場合、卵の中から1段階高い財宝桌面のアイテムが手に入る。",
        ]
...
ダメージが#{getRandomDiff(dice1, 62, d, 1)}以上：深度が1段階低い財宝桌面
ダメージが#{getRandomDiff(dice1, 62, d, 2)}以上：深度が1段階高い財宝桌面
---------",
          "謎の装置のせいで気になるあの子と体が入れ替わってしまった！？シナリオ終了までPCのうち2人の体が入れ替わってしまい、精神的な特徴と癖が入れ替わってしまう。なぜか知力や技能はそのまま。玩家は入れ替わった相手の性格を演じること。入れ替わる2人はランダムに決定するか面白くなるようにGMが選ぶ。展開が面白かった場合、GMはシナリオクリア時のCPを1点追加しても良い。",
          "古代のテクノロジーを使用した医療施設だ。このイベントが発生すると怪獸は消滅する。#{getRandomDiff(dice1, 64, d, 0)}に成功すると全員のHP・FP・MPと転倒以外の状態異常を全回復できる。また、シナリオ終了まで不利な肉体的・精神的な特徴を無効化できる。",
...
----------",
      ...
      when 3
        table = [
          ...
          "急に突風が吹いてきた！全員で#{getRandomDiff(dice1, 36, d, 0)}判定を行うこと。失敗した角色は急斜面を落下して3Dの「叩き」ダメージを受ける。また、「失敗したPCの人数×1D」の深度が減少する。",
        ]
      ...
      when 4
        table = [
          ...
          "突然魔族が飛びかかってきた！#{getRandomDiff(dice1, 44, d, 0)}。体の一部が宝石でできており、普通の#{getRandomDiff(dice1, 44, d, 1)}とは異なるようだ。撃破時に入手できるAPが2倍で、ドロップ判定の骰子が良い方に1点修正される。#{getRandomDiff(dice1, 44, d, 2)}",
      ...
      when 5
        table = [
          ...
          "ファイトー！いっぱーつ！！足場が崩れ出す。#{getRandomDiff(dice1, 52, d, 0)}に失敗したPCは落下して#{getRandomDiff(dice1, 52, d, 1)}の「叩き」ダメージを受ける。成功したPCは体力判定を行い、落下するキャラを支えることができる。この判定に失敗した場合支えようとした角色も一緒にダメージを受ける。",
          "滑りやすい岩を飛び移っていく道が続いている。敵味方全員が#{getRandomDiff(dice1, 53, d, 0)}判定を行い、失敗した角色は転倒してFPに1点のダメージを受ける。浮遊・飛行して移動している角色はこの影響を受けない。#{getRandomDiff(dice1, 53, d, 1)}",
      ...
      when 6
        table = [
          ...
          "不思議な岩がある。1Dを振って1・2であれば火炎、3・4であれば冷気、5・6であれば電撃特性の攻撃で破壊可能。#{getRandomDiff(dice1, 63, d, 0)}行動ターンを消費して岩を狙うと宣言すればよい。岩は全てのヘクスから攻撃可能で、破壊したあとには財宝が発見できる。財宝の中身は現在の深度に応じた財宝桌面を使用する。",
          "ストームコーザーが獲物を取った後調理をしている。このイベントが発生すると怪獸は消滅する。「こんなところで人間に会うなんて珍しいな、お前らも食っていくか？」10GPを支払うことでマジカルクッキング#{getRandomDiff(dice1, 64, d, 0)}の効果を得ることができる。",
    ...
    when 4
      ...
      when 1
        table = [
          ...
          "リザードの集落につく。HPの減った角色がいれば持て囃され、マジカルクッキング「カリカリミミズ肉」をごちそうしてもらい、この先の道案内も買って出てくれる。そのラウンドの深度判定に+3のボーナスを得る。全員のHPが満タンの場合は、そっけない扱いをされ何も起こらない。",
          "回復の泉に見せかけた毒の泉。パーティーメンバーのHP・MP・FPのいずれかが減少している場合このイベントは拒否できない。毒の水でHP・MP・FPに#{getRandomDiff(dice1, 16, d, 0)}のダメージを受ける。HP・MP・FPが全快の角色はこの効果を受けない。#{getRandomDiff(dice1, 16, d, 1)}か「第六感」に成功すればこれが毒の泉だとわかる。",
        ]
      ...
      when 3
        table = [
          ...
          "大蓮のような植物の群生地。荷重レベルが「軽荷」以下の角色がいる場合安全に移動できるようになり深度が1D増加する。全員「並荷」以上の場合は大蓮が絡まり深度が1D減少する。",
      ...
      when 4
        table = [
          "#{getRandomDiff(dice1, 41, d, 0)}とエンカウント。2ラウンド経過するとキャンディークラウンは戦場全域に《べたべた》をかけて逃亡する。",
          "アシガル潜水隊と遭遇。#{getRandomDiff(dice1, 42, d, 0)}とエンカウント。<偽装>か「視覚-3」判定に成功すると水面に動く竹筒を発見可能。最初の判定に成功していればPC側のイニシアチブ値+1、失敗していれば敵の先手で戦闘を開始する。その際PC先手で進行中の場合も怪獸先手に変更される。",
          "何か水面が揺れている。#{getRandomDiff(dice1, 43, d, 0)}。敵は全員【ハイドインシャドウ】状態。#{getRandomDiff(dice1, 43, d, 1)}判定に失敗すると敵の先手で戦闘を開始する。その際PC先手で進行中の場合も怪獸先手に変更される。",
      ...
      when 5
        table = [
          "美しい歌声が響き渡る、意識が引きずり込まれそうだ。その場にいる全員(敵も)は#{getRandomDiff(dice1, 51, d, 0)}判定を行う。音量自動補正装置がある場合この効果を受けない。失敗した場合1ラウンド朦朧状態となる。朦朧とした角色は深度判定も不可。",
          "小舟で進んでいると、なにやら激しい水の音がする。この先は滝のようだ！行動を消費して#{getRandomDiff(dice1, 52, d, 0)}を行い、全員が失敗すると#{getRandomDiff(dice1, 52, d, 1)}の防護点無視ダメージを受ける。この判定がクリティカルだった場合、滝の裏に財宝の洞窟を発見する。罠や鍵はかかっておらず、財宝の中身は1段階高い深度の財宝桌面を使用する。",
      ...
      when 6
        table = [
          ...
          "老齢のグラントが岩に腰かけ釣りをしている。このイベントが発生すると怪獸は消滅する。「どうだね、よかったら釣り勝負をやっていかないか？グラントの太公望(#{getRandomDiff(dice1, 64, d, 0)})とPCの<釣り>技能で行動を消費して即決勝負を行う。成功度で勝利した場合、「見事じゃ、そこにあるガラクタはワシには必要ない物じゃから好きに持っていくがいい」と財宝を渡してくれる。財宝の中身は1段階高い深度の財宝桌面を使用する。",
          "船の墓場だ、ここは何かありそうな感じがする。#{getRandomDiff(dice1, 65, d, 0)}に成功すると「2D×#{getRandomDiff(dice1, 65, d, 1)}」GPを入手できる。判定がクリティカルだった場合、それに加えて財宝も入手可能。財宝の中身は1段階高い深度の財宝桌面を使用する。",
    ...
    when 5
      ...
      when 4
        table = [
          ...
          "魔族の偵察隊と遭遇する。#{getRandomDiff(dice1, 43, d, 0)}とエンカウント。オウルセージのうち1体は行動ターンに逃亡して他の魔族に警戒を呼び掛けに行く。これを阻止できなかった場合、以後出現する「分類：魔族」の怪獸は物理攻撃の致傷力+#{getRandomDiff(dice1, 43, d, 1)}、防護点+#{getRandomDiff(dice1, 43, d, 1)}のボーナスを得る。",
          "木々の間を飛び回る何かに背後から突然襲撃される。#{getRandomDiff(dice1, 44, d, 0)}。ペット「スカイサーチャー」か「広視界」がなければ必ず相手の先制攻撃になり、その際PC先手で進行中の場合も怪獸先手に変更される。",
          ...
          "前方の森に何か違和感を感じる。周囲に擬態していた#{getRandomDiff(dice1, 46, d, 0)}。「視覚-5」か「第六感」の判定に成功していなければ必ず相手の先制攻撃になり、その際PC先手で進行中の場合も怪獸先手に変更される。判定成功時は深度を1D減少させて戦闘回避も可能。",
        ]
      ...
      when 5
        table = [
          ...
          "巧妙なベアトラップが仕掛けてある。ランダムな角色1人(怪獸も含む)に#{getRandomDiff(dice1, 53, d, 0)}の「切り」ダメージを与える。以後ターンの最初に#{getRandomDiff(dice1, 53, d, 1)}の判定に成功するまで移動不可。本人か隣接した角色が行動を消費して#{getRandomDiff(dice1, 53, d, 2)}で判定することでも解除可能。",
          ...
          "すさまじい悪臭を放つ巨大な花。#{getRandomDiff(dice1, 56, d, 0)}判定に失敗した角色はシナリオ終了まで知力が#{getRandomDiff(dice1, 56, d, 1)}点減少する(最大MPは変化しないものとする)また、対応技能が<技師/魔道具>以外のペットは匂いで気絶してシナリオ終了まで効果を得られなくなる。",
        ]

      when 6
        table = [
          "魔族の狩りにより深く傷ついたクレッセントムーンを発見する。<応急処置-7>か<獣医-3>の判定に成功すると、クレッセントムーンの傷は完治し、クレッセントムーン(ペット)を手に入れることができる。",
          "少し開けた原っぱで、フラウのダンスパーティーに誘われる。このイベントが発生すると怪獸は消滅する#{getRandomDiff(dice1, 62, d, 0)}で判定を行い、成功するとそこにキノコが生え始める。フラウは協力してくれたお礼にこのキノコを分けてくれる。大変貴重な珍味で、持ち帰ると「(1D+成功度)×#{getRandomDiff(dice1, 62, d, 1)}」GPになる。",
          "フラウたちが「森ガールファッションコンテスト」を開催しており、その助っ人を依頼される。このイベントが発生すると怪獸は消滅する。#{getRandomDiff(dice1, 63, d, 0)}に成功すると自然と一体化したファッションが高評価を受け、お礼に#{getRandomDiff(dice1, 63, d, 1)}(装飾品)を入手できる。",
          "霊樹の花にたまった水が魔力を帯びている。<神秘学>に成功するとそれにポーションとしての効果があることが分かる。1Dを振り、その出目に対応したポーションが手に入る。
----------
#{getRandomDiff(dice1, 64, d, 0)}
----------",
          "迷子のフラウを見つける。イベント表で他のフラウに出会った場合そのフラウと知り合いで、お礼に財宝を渡してくれる。財宝の中身は1段階高い深度の財宝桌面を使用する。フラウに出会えずシナリオをクリアした場合も、1段階低い深度の財宝桌面の中身をくれる。",
    ...
    when 6
      ...
      when 1
        table = [
          ...
          "ヒガンバナの世話をするフラウと出会う。100GPを払うと「ヒガンバナの毒」を武器に塗ってもらうことができる。最終ダメージに+2の効果があり、武器の通じない「ファントム」のような怪獸にも武器による攻撃が有効になる。この効果はシナリオ終了時まで持続する。",
          "シルヴァテイルのネクロマンサー達が集まって、何やら怪しい儀式をしている。道中で死亡もしくは保険を適用して強制送還された角色がいれば、#{getRandomDiff(dice1, 15, d, 0)}GPを支払うことで復活させてもらうことができる。それ以外の場合、何か不気味な儀式に不安を感じ、FPを1点消耗する。",
      ...
      when 3
        table = [
          "なにもなさそうな道だが、#{getRandomDiff(dice1, 31, d, 0)}で判定を行う。成功するとちょっとなにかでそうな近道を発見する。深度が2D増加するが、「臆病」な角色は意志判定が必要。",
          ...
          "地面から突如骨の手が伸び移動を邪魔する。全員「移動力+5」を目標値に判定を行い、失敗した角色の人数だけ深度が減少する。【死者に鞭】を持つネクロマンサーがいればこの効果は無視できる。",
      ...
      when 4
        table = [
          "#{getRandomDiff(dice1, 41, d, 0)}とエンカウント。2ラウンド経過するとキャンディークラウンは戦場全域に《べたべた》をかけて逃亡する。",
          "朽ちていた骨が突如起き上がり襲いかかる。#{getRandomDiff(dice1, 42, d, 0)}。「第六感」の判定に成功していなければ必ず相手の先制攻撃になり、その際PC先手で進行中の場合も怪獸先手に変更される。",
      ...
      when 5
        table = [
          ...
          "墓石が動き出し中から何かが出てこようとしている。行動を消費して#{getRandomDiff(dice1, 54, d, 0)}で判定を行い、成功すればなにも起こらない。失敗した場合、探索中に倒された怪獸の中で最もCLが高い怪獸と再度エンカウントする(深度判定終了後)該当する怪獸が存在しなかった場合、#{getRandomDiff(dice1, 54, d, 1)}が出現する。",
      ...
      when 6
        table = [
          "雷が突然鳴り響く。その場にいる全ての角色(怪獸含む)は全員3Dを振り、金属鎧を着ている角色はその値に+3する。この値が一番高い角色は電撃により#{getRandomDiff(dice1, 61, d, 0)}の「叩き」ダメージを受ける(金属鎧の防護点を無視)",
          "タルの上に腰かけ酒を口に運び続ける、減ることのない酒に向き合うスケルトンがいる。「どうだ、お前らも飲んでいくか？」と声をかけられる。酒を飲む場合、#{getRandomDiff(dice1, 62, d, 0)}判定を4回連続で行うこと。全て成功した場合は、「ついに酒が切れたか、最後の晩酌に付き合ってもらって感謝する」と感謝を述べて財宝を渡してくれる。財宝の中身は1段階高い深度の財宝桌面を使用する。途中で1度でも失敗した場合、酒に悪酔いしてシナリオ終了まで全ての判定に-1のペナルティを受ける。",
...
----------",
          ...
          "『千ノ戦イデ千ノ勝チ方ヲ知ル者、ココニ眠ル』と刻まれた棺を見つける。「好奇心」「直情」の角色は開けずにいるには意志判定が必要。棺を開けた場合は#{getRandomDiff(dice1, 66, d, 0)}。勝利した場合、通常のドロップとは別に#{getRandomDiff(dice1, 66, d, 1)}(《戻る武器》は無し)が必ず手に入る。
----------
...
----------",
  ...
  ----------', ''],
    ...
    when 2 # 遺跡
      table = [
        ...
        [32, [
          ['<怪獸知識-5>か<地域知識/カベノソト-3>', '<怪獸知識-7>か<地域知識/カベノソト-5>', '<怪獸知識-9>か<地域知識/カベノソト-7>', '<怪獸知識-13>か<地域知識/カベノソト-11>'],
        ]],
        [33, [
          ['に等しい', '×2の', '×3の', '×5の'],
        ]],
  ...
  ----------', '', ''],
        ...
        [56, [
          ['「現在の深度÷2」以下のCLの怪獸', '「現在の深度」以下のCLの怪獸', '「現在の深度×1.5」以下のCLの怪獸', 'CL80以下の怪獸(ネームド以外)'],
        ]],
