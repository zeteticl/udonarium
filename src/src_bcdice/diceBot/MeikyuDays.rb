...
INFO_MESSAGE_TEXT
  ...
  def md_research_table
    table = [
      ...
      '他の迷宮屋の評判を耳にする。パーティから好きな角色1人を選び、その角色に対する《好意》が1点上昇する。',
  ...
  def md_break_table
    table = [
      'アイテムの改善案を出し合ってみる。各角色は、好きな角色1体を選び、1d6を振ってその角色のアイテムスロットから1つをランダムに選ぶ。出た目のアイテムにレベルがあれば、1上昇する。',
      '何気ない雑談が腹の探り合いに発展する。各角色は、好きな角色に対する《好意》と《敵意》を入れ替え、その属性を自由に変更することができる。',
      '好きな単語表からランダムに単語を1つ選ぶ。その部屋にはそれに関係したものがたくさん置いてあるため、出た単語が「好きなもの」に入っている角色は、《気力》を2点得る。',
      '嫌いな人の話題で盛り上がる。各角色は同じ角色に《敵意》を持っている人を1人選び、その人への《好意》を1点上昇させる。',
      '窓の外から報道のヘリコプターがこちらを撮影しているのが見える。格好よく見せるために、各角色は〔魅力〕で難易度13の判定を行う。誰かが成功するたびに《民の声》が1点増加する。',
      '雑談や休息など、思い思いに時間を過ごす。各角色は、好きな角色1体への《好意》を1点上昇させる。',
      '通路の片隅で素材が山を作っているのを見つけた。各角色は〔探索〕で難易度11の判定を行う。誰かが成功するたびに、好きな素材を1種類選び、それを1d6個手に入れる。',
      'チームワークの確認。各玩家は打ち合わせをせずに、一斉にじゃんけんを行う。いちばん出した人が多かった手を出した玩家のPCは、《気力》を2点得る。',
      '仮眠をとって休憩。各角色は〔才覚〕で難易度9の判定を行う。成功すれば《HP》が最大値まで回復する。',
      '各角色は、迷宮化現象に巻き込まれ、身動きがとれない普通の人を1人見つけた。《配下》に加えることができる。',
      '各角色は1d6を振る。出た目の上位2名が唐突に恋に落ちる。同じ目が出て2名をうまく割り出せない場合は、GMの左隣に近い方を優先する。恋に落ちた2人、相手以外に対する《好意》を合計し、その値に対する《好意》に加える。その後、相手以外に対する《好意》をすべて0にする。',
    ]
  ...
  def md_negotiation_table
    table = [
      ...
      '交渉は決裂！　戦闘を行うこと。',
      '交渉は決裂！　戦闘を行うこと。',
      '「贄をささげれば話を聞こう」怪獸の中で最もレベルが高いもののレベルと等しい数だけ何らかの素材を減少すれば、友好的になる。減少させない場合、戦闘を開始すること。',
      '「……お前の趣味、なに？」好きな単語表一個を選び、D66を振る。パーティの中に、その項目を好きなものにしている角色がいれば、友好的になる。そうでなければ戦闘を開始すること。',
      '怪物たちは物欲しそうにこちらを見ている。「肉」の素材を怪獸の数だけ消費するか、【お弁当】【フルコース】1個を消費すれば友好的になる。消費しなければ、戦闘を開始すること。',
      '怪物たちは値踏みするようにこちらを見ている。現金で1d6MC支払えば友好的になる。そうでない場合、戦闘を開始すること。',
      '「何かいいもんよこせ」怪獸の中で最もレベルの高いもののレベル以上の価格のアイテムを消費すれば友好的になる。そうでない場合、戦闘を開始すること。',
      '「面白い話を聞かせろよ」玩家たちは面白い話をすること。GMは面白いと思えば怪獸は友好的になる。面白くなかった場合は戦闘を開始する。',
      '「俺に勝てたら話を聞いてやろう」怪物が力比べを挑んできた。怪獸の中で最もレベルが高いものと、パーティの代表がそれぞれ〔武勇〕で判定を行う。パーティの代表の達成値が怪獸以上であれば友好的になる。負けた場合、もう一度交渉するか戦闘するかを決定すること。',
  ...
  def md_happening_table
    table = [
      ...
      '期待が大きければ失望も大きい。あなたに対して《好意》を持っている角色全員は、あなたに対する《好意》を1点減らす。',
  ...
  def md_fatal_wounds_table
    table = [
      '重要器官を粉砕される。角色は即座に死亡する。',
      '傍目にも分かる致命傷。角色は次の自分の行動処理が終わった時点で死亡する。《HP》の回復でこの死亡を防ぐことはできない。',
      '全身に強い衝撃をうける。〔武勇〕で難易度[5+受けたダメージ]の判定に成功すると、行動不能になる。判定に失敗すると死亡する。',
      '出血多量で意識不明。行動不能になる。この戦闘が終了するまでに《HP》を1以上にしないと、角色は死亡する。',
      '重傷を負い昏睡状態。行動不能になる。このクォーターが終了するまでに《HP》を1以上にしないと、角色は死亡する。',
  ...
  def md_combat_fumble_table
    table = [
      'ぶざまな失敗に熱くなる。攻撃の目標の角色に対して《敵意》を4点得る。',
      '急にお腹が痛くなる！　何か回復アイテムを使うまで攻撃を行えなくなる。怪獸の場合、そのラウンドの終わりに未行動にならなくなる。',
      'アイテムが壊れた！　自分が持っているアイテムの中からランダムに1つを選び、そのアイテムが失われる。怪獸の場合、1d6ダメージを受ける。',
      '敵がいい気になる。行動不能になっていない敵軍角色全ての《HP》を6点回復する。',
      '自分に攻撃が命中！　使用した武器のダメージを自分に与える。',
      'なんというか、やる気をなくす。《気力》を1点失う。怪獸の場合、1d6ダメージを受ける。',
      '仲間に攻撃が命中！　使用した武器の射程内の味方から、ランダムに1人を選ぶ。その角色に武器のダメージを与える。',
      '仲間の邪魔をしてしまう。未行動の自軍角色1体を選び、行動済みにする。',
      'スキルを忘れてしまった！　習得しているスキルからランダムに1種類を選ぶ。そのスキルは戦闘が終了するまで使用できない。',
      '位置取りに失敗してとんでもない場所に。敵陣営玩家またはGMが、ファンブルした角色を好きな位置に移す。',
  ...
  def md_appearance_table
    table = [
      '「ここから先に行かせるわけにはいかん」急ぐ途中に敵が立ちふさがる。〔武勇〕で難易度11の判定を行う。成功すればバトルフィールドの好きなエリアにその角色を配置することができる。失敗した場合、《HP》を1にした状態でバトルフィールドの好きなエリアにその角色を配置することができる。',
      '「待たせたな！」バトルフィールドの好きなエリアにその角色を配置することができる。',
      'おっと鉢合わせ！　バトルフィールドの敵軍の本陣に、その角色を配置すること。',
      '全力で駆けつける！　《HP》を2d6点減少すれば、バトルフィールドの好きなエリアにその角色を配置することができる。',
      'あいつらはこの先に行ったはず！　GMはその角色をバトルフィールドの好きなエリアに配置する。',
      'あの聞き覚えのある音は……！　その角色が【乗騎】を装備していれば、GMはその角色をバトルフィールドの好きなエリアに配置する。',
      '……間に合ったみたいだな。バトルフィールドの中に、その角色に対する《好意》が1点以上ある角色がいれば、同じエリアにその角色を配置することができる。',
      'を！　これはこれは。好きな素材を1個拾う。',
      'いかん！　迷ってしまった。〔探索〕で難易度11の判定を行うこと。成功すればもう一度登場表を振り、結果を適用することができる。',
      'むむむむ？　ここは一度来た道のような……？　疲労して《気力》が1点減少。',
      '……いや、しかしそれどころではない！　その角色は、この戦闘に登場することはできない。',
    ]
    return get_table_by_2d6(table)
  end
  ...
  def md_pc_connection_table
    table = [
      ...
      '対象は、なんとなくあなたの好きな特徴を備えているような気がする。好きなものにちなんだ特徴を1つ設定し、対象の玩家の了解をとること。チャンスは１回だ。ＯＫなら対象への《好意》が1点上昇する。',
      ...
      'あなたと対象は、今まで隠していたが実はつきあっている。対象の玩家の了解を15秒以内にとることができれば、お互いへの愛情の《好意》を4点上昇させることができる。',
    ]
    return get_table_by_2d6(table)
  end
  ...
  def md_treasure1_table
    table = [
      '何もなし。',
      '何もなし。',
      'その怪獸の素材欄の中から、好きな素材を1個。',
      'その怪獸の素材欄の中から、好きな素材を2個。',
      'その怪獸の素材欄の中から、好きな素材を3個。',
      '【お弁当】1個。',
    ]
  ...
  def md_treasure2_table
    table = [
      'その怪獸の素材欄の中から、好きな素材を3個。',
      'その怪獸の素材欄の中から、好きな素材を4個。',
      'その怪獸の素材欄の中から、好きな素材を5個。',
      'ランダムな回復アイテム1個。',
  ...
  def md_treasure3_table
    table = [
      'その怪獸の素材欄の中から、好きな素材を5個。',
      'その怪獸の素材欄の中から、好きな素材を7個。',
      'その怪獸の素材欄の中から、好きな素材を10個。',
