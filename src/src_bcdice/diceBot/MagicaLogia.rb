...
INFO_MESSAGE_TEXT
  ...
  def magicalogia_fortunechange_table
    table = [
      '『挫折』その角色は、自分にとって大切だった夢を諦める。',
      '『別離』その角色にとって大切な人――親友や恋人、親や兄弟などを失う。',
      '『大病』その角色は、不治の病を負う。',
      '『借金』その角色は、悪人に利用され多額の借金を負う。',
      '『不和』その角色は、人間関係に失敗し深い心の傷を負う。',
      '『事故』その角色は交通事故にあい、取り返しのつかない怪我を負う。',
    ]
    return get_table_by_1d6(table)
  end
  ...
  def magicalogia_accident_table
    table = [
      ...
      lambda { return "生ける屍の群れ、地獄の業火、迷宮化……魔法災厄に襲われる。#{magicalogia_random_skill_table_text_only}の判定を行う。成功すると好きな魔素が1点発生する。失敗すると、その人物に対し「運命変転表」を使用する。" },
  ...
  def magicalogia_conspiracy_table
    table = [
      ...
      '幽閉。その人物には、調査者を異次元の牢獄に閉じ込める魔法が仕掛けられていた。この【秘密】が公開されると、調査者の玩家がシーン玩家になるか、クライマックスフェイズになるまで、シーンに登場できなくなる(マスターシーンでの登場の可否はGMが決定できる)。',
  ...
  def magicalogia_cueball_table
    table = [
      ...
      '殺意。その人物は、世の中に対する憎悪に溢れ、無差別に誰かを傷つけようとしていた。GMはサイクルの終わりにランダムにこの【秘密】の持ち主以外のNPCを一人選ぶ。そのNPCが〈断章〉に憑依されていない〈愚者〉なら死亡し、そうでなければ、この【秘密】の持ち主本人が死亡する。このとき、いずれにせよその死亡を望まない角色は「闇」の分野かの中からランダムに特技一つを選び、判定を行うことができる。判定に成功するとその死亡を無効化できる。',
  ...
  def magicalogia_force_field_table
    table = [
      '加護。その人物の頭上には、常に星の恩寵が降り注いでいる。この【秘密】が公開されると、そのシーンに「星」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「星」の魔素を2点獲得する。',
      '変化(へんげ)。その人物は、実は歳を経た動物―狐や狸などが化けた存在である。この【秘密】が公開されると、そのシーンに「獣」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「獣」の魔素を2点獲得する。',
      '超能力。その人物には、ESPやPKなど、潜在的な超能力が眠っている。この【秘密】が公開されると、そのシーンに「力」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「力」の魔素を2点獲得する。',
      '詩人。その人物は、詩神に祝福された存在である。この【秘密】が公開されると、そのシーンに「歌」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「歌」の魔素を2点獲得する。',
      '夢想家。その人物には、大きな理想や実現したいと思っている夢がある。何か大きなことを成し遂げようとしている。。この【秘密】が公開されると、そのシーンに「夢」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「夢」の魔素を2点獲得する。',
      '暗黒。その人物は、呪われた運命を背負っており、世界の破滅を呼び込む可能性を持っている。この【秘密】が公開されると、そのシーンに「闇」の魔素が2点発生する。また、このセッションの間、この人物に対する【運命】が1点上昇した角色は「闇」の魔素を2点獲得する。',
    ]
    return get_table_by_1d6(table)
  end
  ...
  def magicalogia_prise_table
    table = [
      ...
      '敗者は無数の蟲や小動物へと姿を変え、それを使い魔とする。勝者は好きな特技(魂の特技は除く)を一つ選ぶ。そのセッションの間、指定特技がその特技の【精霊召喚】【魔剣召喚】【悪夢召喚】のうち一つを修得する。',
      '敗者は爆発音と共にどこかへと消え、辺りには硫黄のような匂いだけが残る。勝者はそのセッション中に、一度だけ好きなタイミングで「変調」一種を回復することができる。',
      '敗者は奇妙な形をした数枚の金貨へと姿を変える。勝者は倒した相手のランクと等しい魔貨を獲得する',
      ...
      '敗者は妙なる音楽へと変わり、その曲は勝者の心にしみいる。勝者は倒した相手のランクと等しい魔貨を獲得する。',
      '敗者は一輪の花となる。勝者はそのセッション中に、一度だけ自分が振った行為判定のサイコロを振り直す権利を得る。',
      '敗者は言葉のかけらとなっていき、勝者はそこから未来の運命を読み取る。勝者は「事件表」を使用し、その結果を記録しておく。それ以降、そのセッション中のドラマシーンに、同じシーンに登場している角色一人を選び、一度だけその結果を適用できる。',
  ...
  def magicalogia_innner_world_scene_table
    table = [
      ...
      '大勢の人が行き交う雑踏の中。誰も、あなたに関心を払わない。このシーンでは「不干渉」の世界法則を無視して、好きな角色を調査することができる。',
  ...
  def magicalogia_magic_city_scene_table
    table = [
      ...
      '闘技場。魔法使いが、強化した元型や幻獣を競い合わせている。シーン玩家のPCは、一度だけ、基本ルールブックに載っている、ランク4以下の〈書籍卿〉か〈越境者〉に、魔法戦を挑むことができる。勝利すると、「プライズ表」を使用することができる。',
    ]
    return get_table_by_2d6(table)
  end
  ...
  def magicalogia_world_after_dead_scene_table
    table = [
      '寂しげな村。影のような人々が時折行き来する。ここでは、死んだ角色に会うことができる。このシーンに登場しているPCのうち一人は、疵をひとつだけ、克服することができる。',
  ...
  def magicalogia_labyrinth_world_scene_table
    table = [
      ...
      '幾つもの扉と幾つもの分かれ道を進む。おや？いつの間にか、きみは迷ってしまった。シーン玩家以外のPCが、このシーンに登場しようとする場合、≪道≫で判定を行う。失敗したPCは、このシーンに登場できない。',
      ...
      '金貨や銀貨、宝石に王冠、山のような宝物の積み上げられた部屋。番人は見当たらないが……。このシーンに登場しているPCのうち一人は、≪黄金≫で判定を行うことができる。成功すると1D6点の魔貨かランダムに決定した魔素三個を獲得できる。失敗すると、このシーンに登場しているPC全員の【魔力】が1点減少する。誰かが判定を行うと、成否にかかわらずこの効果はなくなる。',
  ...
  def magicalogia_magic_bookshelf_scene_table
    table = [
      ...
      '町の図書館。いつのまにか、人界に戻ってきてしまったようだ。このシーンに登場しているPCのうち一人は、好きな角色を1人選び、事件表を一回振って、その効果を適用する。',
  ...
  def magicalogia_parallel_world_scene_table
    table = [
      'カフェの桌面。FMラジオは、去年解散したはずのバンドの、先日発表した新曲を流している。このシーンに登場しているPCのうち一人は、「時の流れ表」を1回振り、効果を適用する。',
  ...
  def magicalogia_post_apocalypse_scene_table
    table = [
      ...
      '夜。虚憑の群れに付きまとわれる。シーン玩家は≪悪意≫の判定を行う。失敗するとこのシーンの間、あらゆる判定がマイナス2される。',
  ...
  def magicalogia_god_bar_scene_table
    table = [
      ...
      '乾杯のかけ声、酔っ払いの笑い声、あちこちの桌面では大きな話し声…。どこにでもある、酒場の光景だ。',
  ...
  def magicalogia_starlight_scene_table
    table = [
      ...
      'ふと目を覚ます。汽車に揺られていつの間にか眠っていたらしい。夢の中で誰かと会っていた気がしてならない。シーン玩家が≪深淵≫の判定に成功すると、ランダムに選んだアンカーとの運命が1点上昇する。',
      ...
      '窓から銀河を見つめる、黒い外套を着た少女がいる。あなたに気づくとお菓子を差し出し、にこりと笑って去っていく。シーン玩家はランダムに選んだ魔素を1点獲得する。',
      'ノイズ混じりの車内放送が「停車駅」への到着を告げる。しばらくすると「汽車」が止まった。角色は「停車駅」で降りることができる。降りる場合、「停車駅」の世界法則を決定し、その法則に従ってシーンを進めること。',
      '車窓の向こうに、星明りに照らされたススキ野原が青白く広がる。そんな中で、不思議な紋様の書かれた三角標が何本か、ぽつり、ぽつりと立ち尽くしているのが見えた。',
      '気づくと誰もいない。青いビロウドを張った座席だけが、空間をむなしく彩っている。シーン玩家以外のPCが、このシーンに登場しようとする場合、≪静寂≫で判定を行う。失敗したPCは、このシーンに登場できない。',
  ...
  def magicalogia_world_low_add_table
    table = [
      ...
      '世界法則「妖蝶」。この「停車駅」の周辺は、魔素を帯びた不思議な色合いの羽をもつ蝶が生息している。このシーンに登場しているキャラクターは、「刻印百景」の特技で判定に成功すると、任意の魔素を2点獲得できる。失敗すると、ランダムに選んだ変調1つを受ける。',
  ...
  def magicalogia_old_library_scene_table
    table = [
      ...
      'なぜ、君がここに？扉を開けると、そこにはPCのアンカーがいた。シーン玩家のアンカーの中から、ランダムに一人を選ぶ。そのアンカーが登場する。シーン玩家は、そのアンカーに「事件」を行っても、行動済みにならない。',
    ]
    return get_table_by_2d6(table)
  end
