...
MESSAGETEXT
  ...
  def derive_achievement(num_dices, command)
    # rollメソッドの引数は、順に「骰子数」「骰子1個の出目最大値」「結果のソート」「出目振り足しの基準値（0や1にしておけば処理は行われない）」「成功数を数えるときの比較条件」「成功数の基準値」「振り直しをする出目の基準値」
    # 戻り値は、順に「合計した出目」「ロール結果出力用文字列」「出目が1だった骰子の数」「出目が最大値だった骰子の数」「出目の最大値」「成功数」「振り直しすべき骰子の数」
    # 達成数計算の際にはトライアンフ「出目が6だった骰子は達成数2としてカウントする」を考慮する必要があるが、要は「達成数=成功数+出目が最大値だった骰子の数」になる
