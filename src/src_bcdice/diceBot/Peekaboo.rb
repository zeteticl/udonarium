...
持ち物検査が行われる！イノセント全員は、《隠れる/不良9》の判定を行うこと。失敗した角色は、GMがアイテム1個を選んで没収することができる（セッション終了時に返してもらえる）
クラスで流行っている遊びに誘われる。GMは、「遊び」の分野の中からランダムに特技一つを選ぶ。イノセント全員は、その判定を行う。失敗した角色は、【眠気】が1d6点増える。
とても退屈な授業が始まった。イノセント全員は、《いねむり/不良3》の判定を行う。失敗した角色は、【眠気】が1d6+1点増える。
...
今日も今日とて楽しい授業。GMは、「勉強」の分野の中からランダムに特技1つを選ぶ。イノセント全員は、その判定を行う。失敗した角色は【眠気】が1d6点増える。
特に変わったこともなく、おだやかな一日だった。イノセント全員は、【眠気】が1点増える。
今日の体育の時間はハードだった！　GMは、「運動」の分野の中からランダムに特技1つを選ぶ。イノセント全員は、その判定を行う。失敗した角色は、【眠気】が1d6点増える。
自習の時間だ！GMは、「勉強」の分野の中からランダムに特技1つを選ぶ。イノセント全員は、その判定を行う。成功した角色は、1回だけ自由行動ができる。
抜き打ちテストだ！GMは、「勉強」の分野の中からランダムに特技1つを選ぶ。イノセント全員は、-2の修正をつけて、その特技の判定を行う。成功した角色はよい点をゲット！家にかえってそれを親に見せるとおこづかいを1個もらえる。失敗した角色は、親にこっぴどく怒られ、【眠気】が1d6点増えるうえに、それ以降「みんなで遊ぶ」ことが出来なくなる。
...
TABLE_TEXT_END
    ...
    'OET' => {
      ...
      :type => '2D6',
      :table => <<'TABLE_TEXT_END'
謎かけ守護者が門を護っている。未行動の角色は、《クイズ/遊び10》の判定を行うことができる。判定した角色は行動済みになる。失敗した角色は、1点のダメージを受ける。誰かが成功すれば、イベントはクリアできる。
...
トンガリ族の妖精がいる。彼は、先へ行くための通行料として、アイテムを要求してくる。何か好きなアイテム1個を渡すか、未行動の角色は、《お話づくり/遊び9》の判定を行うことができる。判定した角色は行動済みになる。誰かが判定に成功するか、アイテムを渡すかしたら、イベントはクリアできる。
行き止まりだ。先に進む方法が分からない。未行動の角色は、《推理/大人6》の判定を行うことができる。判定した角色は行動済みになる。誰かが成功したら、イベントはクリアできる。
まっくらで、何も見えない部屋だ。一行は不安におちいる。未行動の角色は、《仕切る/友達11》の判定を行うことができる。判定した角色は行動済みになる。誰かが成功すれば、イベントはクリアできる。
ジメジメした通路だ。特に何もしなくても、イベントはクリアだ。誰かがのぞむなら、自由行動を1回行うことができるぞ。
通路が途中で途切れて崖のようになっている。誰かが飛ぶことが出来れば、向こう岸にあるはしごを断崖にかけられそうだけど……。未行動の角色は、《とぶ/運動6》の判定を行うことができる。判定した角色は行動済みになる。誰かが成功すれば、イベントはクリアできる。
まぼろしの部屋だ。各角色の大好物のまぼろしがつぎつぎ現れる。未行動の角色は、《がまん/友達5》の判定を行うことができる。判定した角色は行動済みになる。誰かが成功したら、イベントはクリアできる。
通路がいくつにも分岐している……。未行動の角色のうち1人が《地理/勉強11》、もしくは《絵/遊び5》の判定を行う。判定した角色は行動済みになる。失敗すると、そのオバケ屋敷の部屋数が1d6点上昇する。成功失敗にかかわらず、イベントはクリアできる。
シャドウが見回りをしている。未行動の角色のうち1人が、《隠れる/不良9》の判定を行う。成功すれば、イベントはクリアできる。失敗すると、玩家と同じ人数のシャドウと戦闘を行うこと。勝利すればイベントはクリアできる。
足下からシャドウが現れ、みんなに襲いかかる！玩家と同じ人数のシャドウと戦闘を行うこと。勝利すればイベントはクリアできる。
TABLE_TEXT_END
    },
...
TABLE_TEXT_END
    ...
    'IBT' => {
      ...
      :type => '1D6',
      :table => <<'TABLE_TEXT_END'
悲しい別れ。病院につれていくことができれば、1d6日入院したあとに目覚めます。その間は、行動不能です。目覚めたときに【眠気】も【元気】もすべて回復しますが、スプーキーを見ることができなくなっています。その角色はスプーキーと一緒に冒険を続けることはできません……。
...
魔法暴発！　自分の持っている魔法をランダムに1つ選んで、その効果が発動します。魔法の対象が選べる場合は、スプーキーの玩家が選んで構いません。そして、1d6サイクルの間、行動不能になります。その後、【魔力】が1d6点回復して通常通り行動できるようになります。
TABLE_TEXT_END
    },
...
突如、誰かの役に立ちたいと思いました。このセッションが終わるまでに、そのスプーキーとイノセントのコンビに対して、5人以上の角色が「ありがとう」と言ったら、コンビのもらえる経験点が1点上昇します。
